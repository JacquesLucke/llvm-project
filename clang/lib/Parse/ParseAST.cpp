...
class FunctionFinder : public RecursiveASTVisitor<FunctionFinder> {
  ...
  bool VisitStmt(Stmt *S) {
    // if (!this->wait_for_new_stmt) {
    //   printf("Did not fully handle previous node.\n");
    //   return false;
    // }
    // printf("%s\n", __func__);
    printf("\n\n");
  ...
  // template<typename T>
  // void handled_stmt(T*stmt) {
  //   if (T::classof(stmt)) {
  //     this->wait_for_new_stmt = true;
  //   }
  // }
};
...
void clang::ParseAST(Sema &S, bool PrintStats, bool SkipFunctionBodies) {
  ...
  // auto ADecl = S.getASTContext().getTranslationUnitDecl();
  // ADecl->dump();


  // CallGraph CG;
  // CG.addToCallGraph(ADecl);

  // struct CheckItem {
  //   CallGraphNode *N;
  // };

  // CallGraphNode *N = CG.getRoot();
  // SmallVector<CheckItem> NodesToCheck;
  // NodesToCheck.push_back({N});

  // llvm::DenseMap<Decl *, uint64_t> HashByDecl;


  // while (!NodesToCheck.empty()) {
  //   CheckItem I = NodesToCheck[NodesToCheck.size() - 1];

  //   for (const CallGraphNode::CallRecord &R : I.N->callees()) {
  ...
  // finder.TraverseTranslationUnitDecl(ADecl);
